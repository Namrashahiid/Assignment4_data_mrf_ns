...
```{r}
#| label: tab-regrapport_mod1
#| tab-cap: Regresjonsrapport modell 1
  summary(lm1)
```
...
# Kan være lurt å utvide til to argumenter slik at også
# modellen kan angis
f_model <- function(a_df, a_mod) {
   lm(
      formula = a_mod,
      data = a_df
    )
...
```{r}
#| label: tab-regrapport_rogaland_mod4
#| tab-cap: Regresjonsrapport for fylke Rogaland modell 4
pm2_f %>%
  filter(fnr == "11") %>%
...
pm2_f <- pm2_f %>%
  ...
  mutate(model = map2(.x = data, .y = mod5, .f = f_model)) 
```

```{r}
# Østfold
summary(pm2_f$model[[1]])
```
...
Ifølge @tab-regrapport_rogaland_mod4 ser vi at tallene for knr1102 er større, enn tallene representert fra tabell for Rogaland øverst.
...
```{r}
#| label: fig-utvikling_res_rogaland
#| fig-cap: Plot for utviklingen av residualene for kommunene i Rogaland
pm2_f %>% 
  filter(fnr == "11") %>% 
  ggplot(mapping = aes(x = aar_d, y = resid, colour = knr)) + 
  #geom_point() +
  geom_hline(yintercept = 0) +
...
pm2_f %>%
  unnest(resid) %>%
  group_by(fnr, aar_d) %>%
  summarise(mean_res_m5 = mean(resid)) %>%
  ggplot(mapping = aes(x = aar_d, y = mean_res_m5, colour = fnr
                      )) +
  geom_smooth(se = FALSE) + 
  geom_hline(yintercept = 0) + 
  scale_size_manual() +
...
pm2_f %>% 
  ggplot(mapping = aes(x = aar_d, y = resid)) + 
  # ei linje for kvart fylke, group = fnr
  geom_line(aes(group = fnr, colour = fnr), alpha = 1/6) +
  # general smooth for alle fylker
  geom_smooth(se = FALSE) 
